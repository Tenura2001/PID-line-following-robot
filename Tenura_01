...
void sensor_reading() {
  ...
  // Check surface type and set sensor values accordingly
  if (irL < threshold && irR < threshold) {
    isBlackSurface = true;
  } else if (irL > threshold && irR > threshold) {
    isBlackSurface = false;
  }
...
void Line_Follow() {
  while (1) {
    ...
    // All sensors on white or black surface
    ...
    // All sensors on black or white surface
